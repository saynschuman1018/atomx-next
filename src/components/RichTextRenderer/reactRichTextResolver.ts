...
import { MarkResolvers, defaultMarkResolvers } from './resolver/mark'
import { customBlokResolvers } from './resolver/bloks'
  ...
  if (
    ...
    const renderNodes = (nodes: any) => {
      const elements = nodes
        ? nodes.map(renderNode).filter((node: any) => node != undefined)
        : null
...
export const reactRenderRichText = (document: any, options?:RenderOptionsProps) => (
  render(document, {
    blokResolvers: customBlokResolvers,
    ...options,
  })
)
